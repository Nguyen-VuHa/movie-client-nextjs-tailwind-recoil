...
function ModalCustom({ 
    ...
    return (
        <div 
            className={`fixed top-0 left-0 right-0 z-50 justify-center items-center
            modal-custom
            ${!status ? 'hide' : '' } 
        ...
        >
            ...
            <div className={`relative w-full h-full modal-content ${maxWidth || 'max-w-md'} ${status ? '' : 'hide' } md:h-auto`}>
                <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    {/* Close button */}
                    <button onClick={() => onHidden && onHidden()} type="button" className="z-[2] absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center :is(dark:hover:bg-gray-800) :is(dark:hover:text-white)" data-modal-hide="authentication-modal">
